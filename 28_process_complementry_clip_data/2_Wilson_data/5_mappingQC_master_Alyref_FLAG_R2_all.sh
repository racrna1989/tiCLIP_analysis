...
for INBAM in ${INBAMDIR}"Alyref_FLAG_R2_all.sam";
do

ID=$(basename $INBAM | sed 's/.sam//g')

##sort sam to bam
samtools import ${REFERENCE} ${INBAM} ${OUTMAPQCDIR}${ID}".bam" 
samtools sort -@ 10 ${OUTMAPQCDIR}${ID}".bam" -o ${OUTMAPQCDIR}${ID}".So.bam" 
