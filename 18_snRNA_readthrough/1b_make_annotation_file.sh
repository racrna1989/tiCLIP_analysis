...
awk '{OFS="\t"}{if ($6 =="+"){print $1,$3,$3+1000,$4":::1000nt_downstream",$5,$6} else if ($6 =="-"){print $1, $2-1000,$2,$4":::1000nt_downstream",$5,$6}}' snRNA.GRCh38101_and_Soren_anno.bed > extentions.tmp
awk '{OFS="\t"}{$4=$4":::genebody"; print $0}' snRNA.GRCh38101_and_Soren_anno.bed | cat - extentions.tmp | sort -k1,1 -k2,2n | grep -v "KI27" | grep -v "GL00" > snRNA.GRCh38101_and_Soren_anno.1000ntextention.bed
