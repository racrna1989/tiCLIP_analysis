...
readAndWrangleDataFrame <- function(FILEPATH){
  ...
  DF_1<-DF %>%
    ...
    mutate(sizeRange = case_when( 
      ...
      genomicSize %in% c(200001:210000) ~ "200k-210k",
      genomicSize %in% c(210001:220000) ~ "210k-220k",
      genomicSize %in% c(220001:230000) ~ "220k-230k",
      genomicSize %in% c(230001:240000) ~ "230k-240k",
      genomicSize %in% c(240001:250000) ~ "240k-250k",
      genomicSize %in% c(250001:260000) ~ "250k-260k",
      genomicSize %in% c(260001:270000) ~ "260k-270k",
      genomicSize %in% c(270001:280000) ~ "270k-280k",
      genomicSize %in% c(280001:290000) ~ "280k-290k",
      genomicSize %in% c(290001:300000) ~ "290k-300k",
      genomicSize > 300000 ~ ">300k"
    )) %>% 
    mutate(coding_group = case_when(
      grepl("protein_coding|histone_coding", bioType) ~ "coding",
      grepl("rRNA", bioType) ~ "rRNA",
      TRUE ~ "noncoding"))
...
calculateCoverage <- function(DF) {
  DF_1<- DF  %>%
    ...
    select(Protein, Rep, Timepoint, geneName,TotalBins,coding_group, binNumber,sizeRange, count) %>%
    ...
    group_by(Protein, Rep, coding_group, Timepoint,sizeRange) %>%
    mutate(n=n_distinct(geneName)) %>%
    ungroup() %>%
    group_by(Protein, Rep, Timepoint,binNumber,coding_group, sizeRange,n) %>%
