...
number_of_intron_annotations<-
  ...
  filter(!grepl("snRNA|rRNA|TR_C_gene|IG_C_pseudogene|miRNA|misc_RNA", geneID) & as.numeric(ExonSize) >99) %>%
...
wrangle_bed_counts <- function(dataframe) {
  dataframe %>%
    separate(geneID, into = c("geneID", "Biotype", "IntronNumber", "TotalNumberOfIntrons", "ExonSize", "ExonicDisance", "ExonDistFromTSS","ExonStature", "GeneStructure"), sep=":::") %>%
  	filter(grepl("multi", GeneStructure) & geneID != "LINC00324" & !grepl("snRNA|rRNA|TR_C_gene|IG_C_pseudogene|miRNA|misc_RNA", geneID) & as.numeric(ExonSize) > 99) %>%
