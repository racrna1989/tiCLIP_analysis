...
OUTDIR=${MASTEROUTDIR}"2_counts_of_reads_overlapping_10nt_window_over_5end_or_3end_of_exons/"

ANNODILEREGIONS="annotationFiles/hg38_HeLa_trimmed_loci_major_primary_isoform_annotated_exon_numbered.exonEnds10ntupdown.bed"
...
bedtools intersect -c -s -b ${INBAM} -a <(sed 's/^chr//g;s/|/_/g' $ANNODILEREGIONS | sort -k1,1 -k2,2n )  > ${OUTDIR}${ID}".exonEnds10updown.counts"

done

#for read 2, we can do intersect with -s option as read one is on trans strand. 

for inBam in bams/split_non-split/ALYREF-*read2*
do 
ID=$(basename ${INBAM})
...
for f in ${OUTDIR}ALYREF*Exon*counts; do ID=$(basename $f | sed 's/.counts//g;s/.bam//g;s/\./_/g;s/-/_/g') ; awk -v ID=$ID '{OFS="\t"} $7 > 0 {print ID,$0}' $f >> ${MASTEROUTDIR}"ALYREF_Exon_splice.count" ; done &
for f in ${OUTDIR}ALYREF*spliceSites*counts; do ID=$(basename $f | sed 's/.counts//g;s/.bam//g;s/\./_/g;s/-/_/g') ; awk -v ID=$ID '{OFS="\t"} $7 > 0 {print ID,$0}' $f >> ${MASTEROUTDIR}"ALYREF_spliceSites10updown.count" ; done &
