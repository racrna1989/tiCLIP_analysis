...
for inBedgraph in ${inBedDir}*rRNAScaled.fwd.bedgraph;
	...
	bedtools map -a $bedAnnotationPlus -b <( sort -k1,1 -k2,2n $inBgPlus | bedtools intersect -v -a - -b <(cat $Exons $snRNAs $snoRNAs | awk '{OFS="\t"} $6 =="+"' | sed 's/^chr//g' | sort -k1,1 -k2,2n ) ) -g $genome -c 4 -o sum -null 0 > ${outDir}${ID}"_"${RandPlus}"_"${outFileName}"_tmp_plus" &
	bedtools map -a $bedAnnotationNeg -b <( sort -k1,1 -k2,2n $inBgNeg | bedtools intersect -v -a - -b <(cat $Exons $snRNAs $snoRNAs | awk '{OFS="\t"} $6 =="-"' | sed 's/^chr//g' | sort -k1,1 -k2,2n ) ) -g $genome -c 4 -o sum -null 0 > ${outDir}${ID}"_"${RandNeg}"_"${outFileName}"_tmp_neg" &
		
	wait
	
	name=$(echo $ID | sed 's/\./\t/g' | cut -f 1 )
	
	cat ${outDir}${ID}"_"${RandNeg}"_"${outFileName}"_tmp_neg" ${outDir}${ID}"_"${RandPlus}"_"${outFileName}"_tmp_plus" | awk -v name=$name '{OFS="\t"}{print name, $0}' >> ${outDir}"all_Introns."${outFileName}"sense.counts"
	
	wait

	rm ${outDir}${ID}"_"${RandNeg}"_"${outFileName}"_tmp_neg" ${outDir}${ID}"_"${RandPlus}"_"${outFileName}"_tmp_plus"
