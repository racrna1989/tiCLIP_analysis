...
annoBed %>% 
  separate(geneID, into = c("geneID", "Biotype"), sep = ":::") %>%
  group_by(Biotype) %>%
  summarise(intron_count=n())

#load 5 end count file -----------------
...
#this replaces NAs introduced by no value present in expression_vector, and replaces them with min value in expression_vector
  ...
  group_by(Sample, DistToLandmark, Biotype) %>% 
  summarise(sum_RNAseq_norm_count = sum(norm_count)) %>% 
  ungroup() %>%
  left_join(number_of_intron_annotations) %>%
  mutate(sum_RNAseq_norm_count = sum_RNAseq_norm_count / intron_count,
  Position = "five_end")
