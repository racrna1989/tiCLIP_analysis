...
#corrected annotation counts column 200910
...
ANNOTATION_BED_FILEPATH_TRANSCRIPTS<-args[2]
EXPRESSION_VECTOR_FILEPATH<-args[3]
IN_FILE_PATH_3_END<-args[4]
IN_FILE_PATH_5_END<-args[5]
OUTFILE_PATH<-args[6]
...
#wrangle bed counts --------------------------------------
wrangle_bed_counts <- function(DF) {
col_nums=c("start","end","ExonNumber")
  ...
  DF %>%
    ...
    filter(grepl("multiExonic", GeneStructure)) %>% 
   	filter(geneID %in% transcriptBed$geneID)
...
#3end
wrangle_bed_counts_for_3end <- function(DF) {
  
  DF %>%
    inner_join(DistGroup_3end,  by = c("geneID", "ExonNumber","GeneStructure", "Biotype")) %>% 
    group_by(Sample,GeneStructure, DistToLandmark, Biotype, DistGroup) %>% 
    summarise(sum_RNAseq_norm_count_norm = sum(norm_count)) %>% 
    left_join(number_of_intron_annotations_3end, by = c("DistGroup","GeneStructure","Biotype")) 
    #mutate(sum_RNAseq_norm_count_norm_annotation_number = sum_RNAseq_norm_count_norm_annotation_number/annotation_count)
}


#5end
wrangle_bed_counts_for_5end <- function(DF) {
 
 DF %>%
    inner_join(DistGroup_5end,  by = c("geneID", "ExonNumber","GeneStructure", "Biotype")) %>% 
    group_by(Sample, GeneStructure, DistToLandmark, Biotype, DistGroup) %>% 
    summarise(sum_RNAseq_norm_count_norm = sum(norm_count)) %>% 
    left_join(number_of_intron_annotations_5end, by = c("DistGroup","GeneStructure","Biotype"))
    #mutate(sum_RNAseq_norm_count_norm_annotation_number = sum_RNAseq_norm_count_norm_annotation_number/annotation_count)
}
...
#load annoation bed file containing transcripts---------------------
#new section

transcriptBed <- read.table(ANNOTATION_BED_FILEPATH_TRANSCRIPTS, sep = "\t", header = F) %>% 
  setNames(c("chr", "start", "end", "geneID", "score", "strand")) %>% 
	separate(geneID, into = c("geneID", "Biotype", "TotalNumberOfExons", "geneSize"), sep=":::") %>%
	filter(geneSize < 210000)


#load annobed and count number of annotations-----------------------
...
annoBed[col_nums] <- sapply(annoBed[col_nums],as.numeric)
...
DistGroup_3end<-
  ...
  mutate(three_end = as.numeric(GenomicDistToTSS) + as.numeric(ExonSize)) %>%
    ...
    select(geneID, ExonNumber, GeneStructure, DistGroup, Biotype)

#count groupings for 3 end of exon
number_of_intron_annotations_3end <-  
DistGroup_3end %>% 
  group_by(DistGroup, GeneStructure, Biotype) %>% 
