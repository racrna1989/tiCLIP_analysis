...
col_nums=c("start","end","ExonNumber")
  ...
  DF %>%
    ...
    inner_join(DistGroup_3end,  by = c("geneID", "ExonNumber","GeneStructure")) %>% 
    ...
    left_join(number_of_intron_annotations_3end, by = c("DistGroup","GeneStructure")) %>% 
...
wrangle_bed_counts_for_5end <- function(dataframe) {
  ...
  DF %>%
    ...
    group_by(Sample, GeneStructure, DistToLandmark, DistGroup) %>% 
    summarise(sum_RNAseq_norm_count_norm_annotation_number = sum(norm_count)) %>% 
    left_join(number_of_intron_annotations_5end, by = c("DistGroup","GeneStructure")) %>% 
    mutate(sum_RNAseq_norm_count_norm_annotation_number = sum_RNAseq_norm_count_norm_annotation_number/annotation_count)
}
