...
OUTDIR=${MASTEROUTDIR}"2_counts_of_reads_overlapping_0nt_window_over_5end_or_3end_of_exons_read2/"
...
#bedtools intersect -c -S -b ${INBAM} -a <(sed 's/^chr//g;s/|/_/g' $ANNOFILEEXONS | sort -k1,1 -k2,2n ) > ${OUTDIR}${ID}.Exon.counts
bedtools intersect -c -S -b ${INBAM} -a <(sed 's/^chr//g;s/|/_/g' $ANNODILEREGIONS | sort -k1,1 -k2,2n )  > ${OUTDIR}${ID}.exonEnds0updown.read2.counts
...
for f in ${OUTDIR}*exonEnds0updown.counts; do ID=$(basename $f | sed 's/.counts//g;s/.bam//g;s/\./_/g;s/-/_/g') ; awk -v ID=$ID '{OFS="\t"} $7 > 0 {print ID,$0}' $f >> ${MASTEROUTDIR}"xiCLIP_all_spliceSites0updown.read2.count" ; done &
