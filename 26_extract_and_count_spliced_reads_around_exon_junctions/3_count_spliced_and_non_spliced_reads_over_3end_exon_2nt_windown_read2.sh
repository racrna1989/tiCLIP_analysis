...
OUTDIR=${MASTEROUTDIR}"3_counts_of_reads_overlapping_2nt_window_over_3end_of_exons_read2/"

ANNODILEREGIONS="annotationFiles/hg38_HeLa_trimmed_loci_major_primary_isoform_annotated_exon_numbered.3endofExonand1ntdown.bed"
...
#bedtools intersect -c -S -b ${INBAM} -a <(sed 's/^chr//g;s/|/_/g' $ANNOFILEEXONS | sort -k1,1 -k2,2n ) > ${OUTDIR}${ID}.Exon.counts
bedtools intersect -c -S -f 1.0 -b ${INBAM} -a <(sed 's/^chr//g;s/|/_/g' $ANNODILEREGIONS | sort -k1,1 -k2,2n )  > ${OUTDIR}${ID}.3endofExonand1ntdown.read2.counts
...
for f in ${OUTDIR}*exonEnds0updown.counts; do ID=$(basename $f | sed 's/.counts//g;s/.bam//g;s/\./_/g;s/-/_/g') ; awk -v ID=$ID '{OFS="\t"} $7 > 0 {print ID,$0}' $f >> ${MASTEROUTDIR}"xiCLIP_all_spliceSites.3endofExonand1ntdown.read2.count" ; done 
