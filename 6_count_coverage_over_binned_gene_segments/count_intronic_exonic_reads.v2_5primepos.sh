...
#counts for where protein cross-links
...
OUTDIR="4_geneCount/intron_exon_counts/5primepos/"
...
for INBED in ${INDIR}*5primepos.bed
do
	...
	#count total reads
	wc -l ${INBED} | cut -d " " -f 1 | awk -v name=$OUTNAME '{OFS="\t"}{print name, $0}' >> ${OUTDIR}"xiCLIP.5primepos.totalcounts.200402.tab" &
	...
	awk -v name=$OUTNAME '{OFS="\t"}$7 > 0 {print name,$0}' >> ${OUTDIR}"xiCLIP_5primepos_intronExon.201217.counts" & 
	...
	wait
	cat ${OUTDIR}${OUTNAME}".Intron.counts" ${OUTDIR}${OUTNAME}".Exon.counts" | awk -v name=$OUTNAME '{OFS="\t"}$7 > 0 {print name,$0}' >> ${OUTDIR}"xiCLIP_5primepos_intron_exclusiveExon.201217.counts"
