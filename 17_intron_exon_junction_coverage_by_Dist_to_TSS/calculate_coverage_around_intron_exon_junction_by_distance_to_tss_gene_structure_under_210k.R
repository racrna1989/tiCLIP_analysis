...
#wrangle bed counts --------------------------------------
wrangle_bed_counts <- function(DF, DISTGROUP, N) {
...
col_nums=c("start","end","ExonNumber")
  ...
  DF %>%
    ...
    inner_join(DISTGROUP,  by = c("geneID", "ExonNumber","GeneStructure")) %>% 
    group_by(Sample,GeneStructure, DistToLandmark, DistGroup) %>% 
    summarise(sum_RNAseq_norm_count_norm_annotation_number = sum(norm_count)) %>% 
    left_join(N, by = c("DistGroup","GeneStructure")) %>% 
...
DistGroup_3end<-
  ...
  mutate(three_end = as.numeric(GenomicDistToTSS) + as.numeric(ExonSize)) %>%
    ...
    select(geneID, ExonNumber, GeneStructure, DistGroup)

#count groupings for 3 end of exon-------------------------------------
number_of_intron_annotations_3end <-  
DistGroup_3end %>% 
  group_by(DistGroup, GeneStructure) %>% 
...
#count groupings for 5 end of exon-------------------------------------
...
int_exon_junction_coverage_five_end<-
  wrangle_bed_counts(int_exon_junction_coverage_five_end, DistGroup_5end, number_of_intron_annotations_5end) %>% 
...
int_exon_junction_coverage_three_end <-
  wrangle_bed_counts(int_exon_junction_coverage_three_end, DistGroup_3end, number_of_intron_annotations_3end) %>% 
