...
INPUTGENERICSCRIPT=$INDIR"5a_generic_input_to_run_r_scripts.sh"
...
for DESC in ${INDIR}calculate*210k_biotype.R ; do 
...
sed "s|CHANGEME|$DESC|g;s|ANNOTATIONFILEPATH|$ANNOTATION_FILE_PATH|g;s|SAMPLE_FILE_PATH|$SAMPLE_NAME_FILEPATH|g" $INPUTGENERICSCRIPT > $JOBFILE
echo $JOBFILE
fBatch $JOBFILE
